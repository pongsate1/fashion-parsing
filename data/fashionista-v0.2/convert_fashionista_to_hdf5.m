...
%
  ...
  options.input = fullfile(root, ...
                           'fashionista-v0.2.1', ...
                           'fashionista_v0.2.1.mat');
  options.output = fullfile(root, '%s-%g.h5');
  options.output_list = fullfile(root, '%s-%g.h5.txt');
  options.output_labels = fullfile(root, 'labels.txt');
  ...
  load(options.input, 'truths', 'test_index');
  ...
  exportLabelFile(options.output_labels, labels);
...
%CREATEDATASETS Create HDF5 format.
  ...
  for i = 1:numel(truths)
    ...
    superpixel_labels = single(truths(i).annotation.superpixel_labels) - 1;
    annotation = superpixel_labels(superpixel_map);
