function plotCaffeIterations(filename)
%PLOTCAFFEITERATIONS Take a log file and plot the loss over iterations.
%
% plotCaffeIterations('log/train-fcn.o1962');
%
  patterns = {...
  ...
  plot(validation(:, 2), validation(:, 3));
  %hold on;
  %plot(training(:, 2), training(:, 3));
  %hold off;
end

function extraction = extractIteration(line, pattern)
%EXTRACTITERATION Extract the information from one line.
  extraction = [];
